...
import { createBlobImageStore } from '../../../api/_lib/courses/blobImage.js';
...
describe('Vercel Blob image store', () => {
  it('uploads a public blob and returns the HTTPS url', async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      expect.stringContaining(
        'pathname=courses%2FcrsHpNcr01AbcDefGhiJK%2Fimage.png'
      ),
  ...
  it('downloads image bytes from a public HTTPS url', async () => {
    ...
    const image = await store.download(BLOB_URL);
    expect(image?.mimeType).toBe('image/png');
    expect(Buffer.from(image?.bytes || []).toString()).toBe('png-bytes');
  });

  it('ignores non-https image ids on download and delete', async () => {
  ...
  it('deletes all blobs for a course id', async () => {
    const extra =
      'https://store123.public.blob.vercel-storage.com/courses/crsHpNcr01AbcDefGhiJK/image-old.png';
